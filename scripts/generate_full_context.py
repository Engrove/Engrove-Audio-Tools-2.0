...
#
# === SYFTE & ANSVAR ===
# Detta skript bygger den primära context.json-filen genom att hämta all
# relevant data från GitHub-repot. Det är hjärtat i AI Context Builder-verktyget.
#
# HISTORIK:
...
#   den kritiska kärninstruktionen inte kan hämtas.
...
BINARY_EXTENSIONS = {
    ...
    'woff', 'woff2', 'eot', 'ttf', 'otf', 'zip', 'gz', 'tar', 'rar', '7z',
    'pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'mp3', 'wav', 'ogg',
    'mp4', 'mov', 'avi', 'mkv', 'pyc', 'pyd', 'so', 'dll', 'exe', 'bin', 'dat'
}
...
def get_session_headers():
    ...
    headers = {'User-Agent': 'Python-Context-Generator/11.0', 'Accept': 'application/vnd.github.v3+json'}
    if token:
...
def main():
    ...
    for i, item in enumerate(file_list):
        ...
        file_data = {
            "type": "file", "path": path, "size_bytes": item.get('size'),
            "file_extension": file_extension, "is_binary": is_binary or is_json,
            "comments": [], "dependencies": [], "content": None
        }

        if not (is_json or is_binary):
    ...
    final_context = {
        "project_overview": {
            "repository": REPO, "description": repo_info.get('description'),
            "primary_language": repo_info.get('language'), "last_updated_at": repo_info.get('pushed_at'),
